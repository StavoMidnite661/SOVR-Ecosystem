...
async function verifyBurn(txHash) {
  ...
  if (amount !== 10000) throw new Error('Amount mismatch for 10,000 $SOVRCR1 burn');
...
export default async (req, res) => {
  ...
  const { txHash, psik_auth, manualApprove, bmoRouting, bmoAccount } = req.body;
  if (!txHash || !bmoRouting || !bmoAccount || psik_auth !== 'TRUST-AUTH-SOVR') {
  ...
  try {
    ...
    const payout = await stripe.payouts.create({ // Fixed stripeClient to stripe
      amount: amount * 100, // USD to cents (1:1 rate, adjust if different)
      currency: 'usd',
      method: 'standard',
      destination: {
        type: 'bank_account',
        routing_number: bmoRouting,
        account_number: bmoAccount,
      },
      description: `SOVRCR1 Burn ${txHash} - ${checkNumber} to BMO`,
    });
    ...
    return res.status(200).json({
      success: true,
      message: 'ACH payout initiated to BMO.',
      tx_id: txId,
      data: payout,
    });
  } catch (err) {
